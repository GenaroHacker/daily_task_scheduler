...
class SeasonThemeManager:
    ...
    def manage_theme(self):
        ...
        if current_theme is None:
            ...
            Director().print(
                f"Season: {self._get_current_season()}\nTheme: {current_theme}"
            )
            sleep(1)
    ...
    def _set_hemisphere(self):
        ...
        with sqlite3.connect(self.DB_PATH) as conn:
            ...
            cursor.execute(
                "UPDATE season_dates SET is_selected = TRUE WHERE hemisphere = ?",
                (hemisphere,),
            )
            conn.commit()
    ...
    def _get_current_season(self):
        with sqlite3.connect(self.DB_PATH) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT hemisphere, spring_start, summer_start, autumn_start, winter_start FROM season_dates WHERE is_selected = TRUE"
            )
    ...
    def get_current_theme(self):
        ...
        with sqlite3.connect(self.DB_PATH) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT theme FROM themes WHERE season = ? AND year = ?", (season, year)
            )
    ...
    def _update_theme(self):
        ...
        with sqlite3.connect(self.DB_PATH) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO themes (season, year, theme) VALUES (?, ?, ?)",
                (season, year, new_theme),
            )
            conn.commit()
