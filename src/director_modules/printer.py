...
class DynamicTextPrinter:
    def __init__(self, max_line_length=80, char_delay=0.04, acceleration_factor=2.0, words_data_path='assets/data/top_1000_words.json'):
    ...
    def load_common_words(self, path):
        ...
        factor_range = self.acceleration_factor - 1  # Range between most and least common word's factors

        # Calculate speed factors for each word using linear interpolation
        factors = {word: 1 + factor_range * (1 - rank / 999) for rank, word in enumerate(words)}
