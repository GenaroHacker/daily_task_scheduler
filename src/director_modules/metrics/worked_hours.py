...
def plot_start_and_end_hours(db_path, days_range=None):
    ...
    daily_start_hours = df[df['action_type'] == 'start'].groupby('day')['hour'].min().reset_index()
    daily_end_hours = df[df['action_type'] == 'end'].groupby('day')['hour'].max().reset_index()
    ...
    ax.scatter(daily_start_hours['day'], daily_start_hours['hour'], color='blue', label='Start Hours')
    ax.scatter(daily_end_hours['day'], daily_end_hours['hour'], color='red', label='End Hours')
    
    # Plot the lines, handling any missing days for start hours
    for i in range(len(daily_start_hours) - 1):
        if (daily_start_hours['day'].iloc[i + 1] - daily_start_hours['day'].iloc[i]).days == 1:
            ax.plot([daily_start_hours['day'].iloc[i], daily_start_hours['day'].iloc[i + 1]], 
                    [daily_start_hours['hour'].iloc[i], daily_start_hours['hour'].iloc[i + 1]], 'b-')
    
    # Plot the lines, handling any missing days for end hours
    for i in range(len(daily_end_hours) - 1):
        if (daily_end_hours['day'].iloc[i + 1] - daily_end_hours['day'].iloc[i]).days == 1:
