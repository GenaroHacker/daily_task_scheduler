...
def plot_task_time_distribution(db_path):
    ...
    def consolidate_small_tasks(data, threshold=0.02):
        ...
        consolidated_data = consolidated_data[consolidated_data / total_time >= threshold]
        if others > 0:
    ...
    # Hardcoded colors
    color_list = ['#ff0000', '#00ffff', '#7fff00', '#7f00ff', '#ff00aa', 
                '#ffaa00', '#00ff55', '#0054ff', '#2aff00', '#2a00ff']

    # Assign colors to labels
    all_labels = list(set(daily_total_time.index) | set(weekly_total_time.index) | set(monthly_total_time.index))
    colors = {label: color_list[i % len(color_list)] for i, label in enumerate(all_labels)}
    ...
    def plot_pie(ax, data, title):
        ...
        wedges, texts, autotexts = ax.pie(data, startangle=140, autopct='%1.1f%%', colors=colors_for_pie)
        ax.axis('equal')  # Equal aspect ratio ensures that pie is drawn as a circle.
        ax.set_title(title)
        ax.legend(wedges, data.index, title="Tasks", loc="center left", bbox_to_anchor=(1, 0, 0.5, 1))
