...
class GettingThingsDone:
    ...
    def capture_items(self):
        self.d.print("Enter all items on your mind one by one. Type 'end' when you are finished capturing.")
        while True:
    ...
    def process_quick_tasks(self):
        self.d.clear()
        for item in self.actionables:
            if self.input_yes_no(f"Does the item '{item}' take less than 2 minutes to complete? (y/n): "):
                self.quick_tasks.append(item)

    def process_non_actionables(self):
        self.d.clear()
        for item in self.non_actionables:
            if self.input_yes_no(f"Do you want to keep the item '{item}' as reference? (y/n): "):
    ...
    def review_references(self):
        ...
        while True:
            ...
            choice = input("Type a reference ID to update/delete, or type 'end' to finish: ")
            if choice.lower() == 'end':
                break
            if choice.isdigit() and any(ref_id == int(choice) for ref_id, _ in refs):
                action = input("Type 'update' to update or 'delete' to delete the reference: ")
                if action.lower() == 'update':
                    new_ref = input("Enter the new reference: ")
                    cursor.execute("UPDATE refs SET reference = ? WHERE id = ?", (new_ref, choice))
                    conn.commit()
                    self.d.print("Reference updated.")
                elif action.lower() == 'delete':
                    cursor.execute("DELETE FROM refs WHERE id = ?", (choice,))
                    cursor.execute("UPDATE refs SET id = id - 1 WHERE id > ?", (choice,))
                    conn.commit()
                    self.d.print("Reference deleted.")
                # Refresh the list after update or delete
                refs = cursor.execute("SELECT id, reference FROM refs ORDER BY id ASC").fetchall()
                self.d.clear()
            else:
